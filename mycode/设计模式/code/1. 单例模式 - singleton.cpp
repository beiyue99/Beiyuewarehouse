...
public:
    ...
    static TaskQueue* getInstance()
    {
        ...
        {
            m_mutex.lock();
            task = m_taskQ.load();      //这时候取出来的，就不会是还没赋值的内存了
            if (task == nullptr)
            {
                task = new TaskQueue;          //由于底层三步顺序可能不一样，会出现问题，因此引入原子变量
                m_taskQ.store(task);           //设置底层先开辟，再存入内存，最后才赋值给task，这样task不会为空内存了 
            }
            m_mutex.unlock();
        }
        return task;
...
atomic<TaskQueue*> TaskQueue::m_taskQ;         //不需要赋值，存变量调用原子变量的store，取数据调用load方法
