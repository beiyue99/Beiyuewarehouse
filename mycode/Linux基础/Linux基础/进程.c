...
fork()作用是创建一个子进程，会输出两个hello world
 ...
 //fork函数在子进程中返回0，在父进程中返回子进程的pid
  ...
  8     if(pid==0)
  9     {
 10         printf("子进程的pid为：%d,其父进程的pid为：%d\n",getpid(),getppid());
 11         return 0;
 12     }
 13     else
 14     {
 15         printf("父进程的pid为：%d,其子进程的pid为：%d\n",getpid(),pid);
 16     }
 17     return 0;
 18 }

父进程的pid为：50271,其子进程的pid为：50272
子进程的pid为：50272,其父进程的pid为：50271
...
fork()遵从“读时共享，写时拷贝”  如果父进程修改变量的值，会拷贝一份修改，子进程再读取这个变量还是原来的值
  ...
  默认调试父进程 
  (gdb) set follow-fork-mode child 在fork语句前这样设置可以调试子进程
...
1：
       int execl(const char *pathname, const char *arg, ...
                       /* (char  *) NULL */);
 第一个参数为可执行文件的绝对路径和相对路径，第二个参数为可执行文件的名字，然后是可执行文件的参数，最后一个是NULL，与execlp类似，除了第一个参数不同，功能相同   
