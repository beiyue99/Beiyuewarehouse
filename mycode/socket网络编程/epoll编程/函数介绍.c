...
typedef union epoll_data //这个是储存用户数据,当对应的事件触发，内核会传出一个epoll_event，里面的date就是我们传入的date
{
    void* ptr;
    int fd;
    int uint32_t u32
...
在epoll_event结构体中，events字段表示感兴趣的事件类型，可以是以下值之一或它们的组合：
EPOLLIN：表示有数据可读（包括新连接请求）。
EPOLLOUT：表示可以写入数据。
EPOLLRDHUP：表示对端关闭连接，或者半关闭的情况。
EPOLLERR：表示发生错误。
EPOLLHUP：表示发生挂起事件。
EPOLLET：表示使用边缘触发模式。
EPOLLONESHOT：表示一次性监听，监听到事件后需要重新设置。
epoll的优势在于它采用了事件驱动的方式，不需要遍历所有的文件描述符来检查是否有事件发生，
因此对于大量的文件描述符和高并发的场景具有更好的性能。
...
在水平模式下，如果文件描述符处于就绪状态，epoll_wait() 函数将一直返回该文件描述符的就绪事件，直到该事件被处理（即读取完数据或写入完数据）。
水平模式适用于非阻塞 I / O，对于每个就绪的文件描述符，需要循环读取或写入数据直到没有数据可读或可写。
一个读事件，如果一次性没有读完，后续仍然会继续触发事件

边缘模式（Edge - Triggered）：
当文件描述符的状态发生变化时，会触发 epoll 事件。
在边缘模式下，epoll_wait() 函数只会返回文件描述符状态变化的事件，不会重复返回已经就绪的事件。
边缘模式适用于阻塞 I / O，可以使用非阻塞模式或多线程来处理就绪事件。处理完一个事件后，需要重新调用 epoll_wait() 获取下一个事件。
...
输出将包含这个端口的详细信息，包括它的状态（如 LISTEN）和服务名。
如果你找到了占用该端口的进程，但是 netstat 并没有显示 PID / 程序名，那么可以使用 lsof 命令来查找占用特定端口的进程的 PID。
例如，我们可以运行以下命令来查找监听端口 8080 的进程的 PID：
lsof - i :8080
一旦你知道了进程的 PID，就可以使用 kill 命令来停止它。例如，如果进程的 PID 是 1234，那么可以运行：kill 1234
kill - 9 1234  强制杀死s
