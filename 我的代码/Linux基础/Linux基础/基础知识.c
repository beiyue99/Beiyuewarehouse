...
在命令模式下按下冒号（:）进入命令行模式。
输入以下命令：w xx
按下回车键，Vim将保存当前打开的文件为名为xx的文件。
请注意，在此操作之后，当前打开的文件名将保持不变，除非您将其另存为另一个名称或使用“另存为”功能。

如果您想要将文件保存为另一个名称并打开它，可以使用以下命令：

在命令模式下按下冒号（ : ）进入命令行模式。
输入以下命令：w new_filename
按下回车键，Vim将保存当前打开的文件为名为new_filename的文件。
输入以下命令： : e new_filename
按下回车键，Vim将关闭当前文件并打开新保存的文件new_filename。
...
重启虚拟机后，打开终端并输入命令“vmware-hgfsclient”，如果返回文件夹列表，则说明共享文件夹已经正确安装。

在虚拟机的终端中，输入命令“sudo mkdir / mnt / hgfs”，创建一个新的挂载点。
sudo vmhgfs-fuse .host:/share /mnt/hgfs 
或者输入命令“sudo vmhgfs-fuse .host:/ /mnt/hgfs -o allow_other”，将共享文件夹挂载到新创建的目录下。



/ etc / resolvconf / resolv.conf.d / base 文件包含了系统的 DNS 配置信息，当系统需要解析域名时，它将使用该文件中的信息来查找相应的 IP 地址。
...
使用which 可以查找指定文件的路径
man命令里 b前滚一屏，f后滚一屏   （vim编辑器下需要在加上Ctrl）
man 相比help跟安全
find加路径加type加类型
find+路径+-size+范围
find+路劲+ -name +文件名
...
在当前目录及其子目录中查找所有.txt文件并输出到一个文件中：find . - name "*.txt" - print > output.txt
查找最近30天内修改过的所有.log文件：find / path / to / search - name "*.log" - mtime - 30
...
grep - r 加要查找的字符串 加路径        
如果不是目录，可以不加-r，如果要完全匹配这个字符串，需要加-w，如果要查找不存在该字符串的内容，需要加上-v，
如果不在意大小写，需要加上-i，加上-n是显示行号

mkdir -p a/b/c  创建目录树
ifconfig ། grep 加字符串    管道用法
...
file命令可以查看文件或者目录等的类型
du可以查看文件大小，用-s不显示单位，加上-h显示单位
ls   -h显示文件和目录大小,需要配合-l一起使用
...
mgg，m为指定行数，表示回到第n行处
按o是在下一行插入，按O是在上一行插入。(n)yy是复制当前行  p是粘贴  n是行数。
复制时包括光标所在行，而粘贴时，粘到光标下一行。
x是删除光标后内容 X是删除光标前内容 也可以指定删除个数n。
dd是剪切行 默认一行 可以指定参数n。
.是执行上次操作，100加.是执行上次操作100次，u是撤销，Ctrl加r是反撤销。v是选择，shift加v是行选，Ctrl加v是列选。
按/再输入字符串可以查找该字符串，然后按n可以转至下一个查找结果。
r替换当前光标所在的一个字符，R连续替换光标及后面的字符。
末行模式下，如果没修改文件，可以按q退出，如果修改过了，不可以按q，可以按q！强制退出但不保存修改。
末行模式下，按w 加 文件名可以将内容保存到指定文件，如果文件已经存在，需要用w！，此时会覆盖原有文件的内容。
(1) :wq 强制性写入文件并退出，即使文件没有被修改也强制写入，并更新文件的修改时间。
...
cp -r test newtest
如果复制目录，需要加上-r
cp passwd ./d     如果d不存在，会生成一个跟passwd一样的文件，命名为d
